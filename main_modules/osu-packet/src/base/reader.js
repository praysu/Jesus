...
class Reader {
   ...
   * @return {Object|Array}
   */
  Read(buff, layout, requires = null, data = {}) {
    switch (layout.type.toLowerCase()) {
      ...
      case 'multislots': {
        ...
        for (let a = 0; a < 16; a++) {
          data[a] = {};
          data[a].status = buff.ReadByte();
        }
        for (let b = 0; b < 16; b++) {
          data[b].team = buff.ReadByte();
        }
        for (let c = 0; c < 16; c++) {
          if ((data[c].status & (4 | 8 | 16 | 32 | 64)) > 0) {
            data[c].playerId = buff.ReadInt32();
          } else {
            data[c].playerId = -1;
          }
        }
  ...
  UnmarshalPacket(raw, layout = null) {
    ...
    if (layout instanceof Array) {
      layout.forEach(item => {
        if(item.uses) {
          this.Read(buff, item, item.requires ? data[item.requires] : null, item.uses ? data[item.uses] : null);
        } else {
          data[item.name] = this.Read(buff, item, item.requires ? data[item.requires] : null);
        }
      });
